# Shine vs Python vs C# vs Rust benchmark

This suite runs the same algorithms and workload sizes in all four languages:
...
Shine 0.1.3 still uses its tree-walking evaluator. Rust is compiled by `rustc` with `opt-level=3` and one codegen unit, C# uses the .NET 10 Release JIT, and Python uses the installed CPython interpreter. Therefore this measures the current end-to-end runtimes, not the planned Shine LLVM AOT backend. Run on an idle machine, keep it connected to power, close heavy applications, and compare medians rather than one result.
