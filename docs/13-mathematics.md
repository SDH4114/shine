...
Сумма и среднее для `Float` используют компенсированное сложение, чтобы меньше терять малые значения рядом с очень большими. Дисперсия и стандартное отклонение вычисляются устойчивым однопроходным алгоритмом. Для `Int` сумма и произведение остаются точными и завершаются `Value Error` при переполнении.
...
Сравнение `Int` и `Float` не превращает большой `Int` в округлённый `Float`: целые числа больше `2^53` сравниваются без ложного равенства. `round(value)` также выдаёт ошибку, если результат не помещается в `Int`, вместо скрытого ограничения до минимального или максимального значения.
